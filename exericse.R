...
water_ghost_types <- pokemon %>% filter(
  (pokemon$Type.1 == "Water" | pokemon$Type.1 == "Ghost") & pokemon$Speed > "50"
)

# Find the average HP, median HP, min HP and max HP for each of the type of generations
hp_stats_per_gen <- pokemon %>% group_by(Generation) %>% summarise(mean(HP), median(HP), min(HP), max(HP))
...
pokemon %>% filter(Attack > Defense) %>% group_by(Legendary) %>% count()

# Find the generation that has the most number of pokemons with Type.1 as "Fire"
pokemon %>% filter(Type.1 == "Fire") %>% group_by(Generation) %>% count() %>% arrange(desc(n)) %>% head(1) 
...
pokemon %>% filter(Total == max(Total)) %>% head(1)
